...
{post["content"]}
    ...
    <script>
        ...
        function updateLoveUI() {{
            var liked = localStorage.getItem('love_' + postSlug);
            var count = localStorage.getItem('love_count_' + postSlug) || '0';
            var btn = document.querySelector('.love-btn');
            var label = document.getElementById('loveCount');
            if (liked) {{ btn.classList.add('liked'); }} else {{ btn.classList.remove('liked'); }}
            label.textContent = count + ' ဦး ချစ်ကြတယ်';
        }}
        ...
        function addComment() {{
            ...
            if (!name || !text) {{ alert('နာမည်နဲ့ မှတ်ချက် ဖြည့်ပါ'); return; }}
            var comments = JSON.parse(localStorage.getItem('comments_' + postSlug) || '[]');
            comments.push({{ name: name, text: text, date: new Date().toLocaleString('my-MM') }});
        ...
        document.getElementById('viewCount').innerHTML = '👁️ <span>' + views + '</span> ဦး ဖတ်ရှုပြီးပါပြီ';
