...
{post["content"]}
    ...
    <script>
        ...
        function readData() {{
            try {{
                return JSON.parse(localStorage.getItem('post_data_' + postSlug) || '{{}}');
            }} catch (error) {{
                return {{}};
            }}
        }}
        ...
        function writeData(data) {{
            localStorage.setItem('post_data_' + postSlug, JSON.stringify(data));
        }}
        ...
        function renderState() {{
            var state = getState();
            document.getElementById('viewCount').innerHTML = '👁️ <span>' + state.views + '</span>';
            var list = document.getElementById('commentList');
            if (!state.comments.length) {{
                list.innerHTML = '<p style="color:#666;font-size:0.9rem">မှတ်ချက် မရှိသေးပါ</p>';
                return;
            }}
            list.innerHTML = state.comments.map(function(comment) {{
                return '<div class="comment-item"><span class="comment-author">' + comment.name + '</span><span class="comment-date">' + comment.date + '</span><div class="comment-text">' + comment.text + '</div></div>';
            }}).reverse().join('');
        }}
        ...
        function addComment() {{
            ...
            if (!name || !text) {{
                alert('နာမည်နဲ့ မှတ်ချက် ဖြည့်ပါ');
                return;
            }}
            var data = readData();
